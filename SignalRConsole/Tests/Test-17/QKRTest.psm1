...
function Get-Description($qkr)
{
	...
	{
	"`tTo test QKR, run Reset-Test `$true then Start-TestFor Bruce 1, when it finishes,
	connect as Fred on QKR and add Bruce, then run Start-TestFor Bruce 2, check
	friendship status and pop to Home, verify Bruce exits.
	
	Next, connectg as Bruce on QKR, add Fred then pop to Home. Run Start-TestFor Fred,
	connect again as Bruce and accept friend request. Verify status and pop to Home
	then verify that Fred has exited. Run Check-Test `$true to validate the test.`n"
	}
}
