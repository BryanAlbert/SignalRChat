...
function Get-Description($qkr)
{
	...
	connect as Fred on QKR, run Start-TestFor Bruce 2, accept friend request, pop to Home and
	Sverify Bruce exits.
	
	Next connect as Bruce on QKR, add Fred then pop to Home. Run Start-TestFor Fred, conntect as
	Bruce again, verify that Fred is online and pop to Home, verify that Fred exits. Check
