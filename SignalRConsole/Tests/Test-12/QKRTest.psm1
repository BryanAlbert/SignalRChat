...
function Get-Description($qkr)
{
	...
	on QKR, add Fred (fred@gmail.com), check the Status message and pop to Home.
	
	Next, connect as Fred on QKR and run Start-TestFor Bruce in the console. Deny the
	friend request in QKR, check the Status message, close QKR and verify that Bruce
	exits. Run Check-Test `$true to validate the test.`n"
	}
}
