...
function Get-Description($qkr)
{
	...
	{
	"`tTo test QKR, run Reset-Test `$true then run Start-TestFor Bruce 1 and wait for
	it to finish. Connect as Fred on QKR, run Start-TestFor Bruce 2, verify the Status
	message and pop back to Home, verify that Fred exits.

	Next, connect as Bruce on QKR, unfriend Fred, verify the Status message and pop
	back to Home. Run Start-TestFor Fred. Connect on QKR again, verify that Fred is
	not a friend, verify that Fred exits and close QKR. Run Check-Test `$true to
	validate the test.`n"
	}
}
