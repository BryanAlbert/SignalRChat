...
{
	"`n${test}: Bruce adds Fred, Fred accepts
	...
	{
	 "`tTo test QKR, run Reset-Test `$true then run Start-TestFor Fred, connect as
	Bruce on QKR and add Fred (fred@gmail.com), verify friendship, pop to Home,
	verify Fred exits.
	
	Next connect as Fred on QKR and run Start-TestFor Bruce, accept friend request,
	verify friendship, that Bruce exits, then pop to Home and exit QKR. Check
