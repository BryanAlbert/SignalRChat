...
function Get-Description($qkr)
{
	...
	Fred on QKR and accept friend request, verify friendship, that Bruce exits and
	pop to Home.
	
	Next connect as Bruce on QKR, add Fred (fred@gmail.com) then run Start-TestFor
	Fred. Verify that new friend Fred is online, pop to Home, verify that Fred exits
	and close QKR. Check results with Check-Test `$true.`n"
	}
}
