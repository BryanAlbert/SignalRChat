...
function Get-Description($qkr)
{
	...
	Fred on QKR and accept friend request, verify Bruce exits and pop to Home.
	
	Next connect as Bruce on QKR, add Fred then run Start-TestFor Fred. Verify that
	Fred is online, pop to Home, verify that Fred exits. Check results with
	Check-Test `$true.`n"
	}
}
